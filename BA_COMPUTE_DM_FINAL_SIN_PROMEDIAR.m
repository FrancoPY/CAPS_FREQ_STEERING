...
P_ref_L_mean = P_ref_L_mean(idxStart:idxEnd, :);
P_ref_H_mean = P_ref_H_mean(idxStart:idxEnd, :);
P_sam_L_mean = P_sam_L_mean(idxStart:idxEnd, :);
P_sam_H_mean = P_sam_H_mean(idxStart:idxEnd, :);

refMax = max(P_ref_L_mean(:));
P_ref_L_Bmode = 20*log10(P_ref_L_mean / refMax);
P_ref_H_Bmode = 20*log10(P_ref_H_mean / refMax);
P_sam_L_Bmode = 20*log10(P_sam_L_mean / refMax);
P_sam_H_Bmode = 20*log10(P_sam_H_mean / refMax);

% Crear carpeta para guardar las figuras
figDir = fullfile(basedir, 'figuras_v4');
if ~exist(figDir, 'dir'); mkdir(figDir); end

% Visualización B/A
fig1 = figure('Visible', 'off');   % <-- 'off' porque no hay pantalla en el cluster
imagesc(x*1000, z_crop*1000, B_final);
axis image; colormap(turbo); colorbar;
title(sprintf('Mapa B/A (con steering) de frecuencia %s y angulo %s',freqStr, angleStr));
xlabel('Posición Lateral (mm)'); ylabel('Profundidad (mm)');
clim([5 12]);

% Guardar como PNG
outNamePNG = fullfile(figDir, sprintf('BA_freq_%s_angle_%s.png',freqStr, angleStr));
...
title(sprintf('B-mode Referencia Low a frecuencia %s y angulo %s',freqStr, angleStr));
xlabel('Posición Lateral (mm)'); ylabel('Profundidad (mm)');

subplot(2,2,2); imagesc(x*1000, z_crop*1000, P_ref_H_Bmode);
axis image; colormap gray; colorbar; clim([-dynamicRange 0]);
title(sprintf('B-mode Referencia High a frecuencia %s y angulo %s',freqStr, angleStr));
xlabel('Posición Lateral (mm)'); ylabel('Profundidad (mm)');

subplot(2,2,3); imagesc(x*1000, z_crop*1000, P_sam_L_Bmode);
axis image; colormap gray; colorbar; clim([-dynamicRange 0]);
title(sprintf('B-mode Muestra Low a frecuencia %s y angulo %s',freqStr, angleStr));
xlabel('Posición Lateral (mm)'); ylabel('Profundidad (mm)');

subplot(2,2,4); imagesc(x*1000, z_crop*1000, P_sam_H_Bmode);
axis image; colormap gray; colorbar; clim([-dynamicRange 0]);
title(sprintf('B-mode Muestra High a frecuencia %s y angulo %s',freqStr, angleStr));
xlabel('Posición Lateral (mm)'); ylabel('Profundidad (mm)');

% Guardar como PNG
outNamePNG2 = fullfile(figDir, sprintf('Bmode_freq_%s_angle_%s.png',freqStr, angleStr));
