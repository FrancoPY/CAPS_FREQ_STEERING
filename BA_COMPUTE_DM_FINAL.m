...
BA_mean = BA_mean/length(angles);
P_ref_L_meanAngles = P_ref_L_acumu / length(angles);
P_ref_H_meanAngles = P_ref_H_acumu / length(angles);
P_sam_L_meanAngles = P_sam_L_acumu / length(angles);
P_sam_H_meanAngles = P_sam_H_acumu / length(angles);
...
title(sprintf('B-mode Referencia Low a frecuencia %s', freqStr));
xlabel('Posición Lateral (mm)'); ylabel('Profundidad (mm)');

subplot(2,2,2); imagesc(x*1000, z_crop*1000, P_ref_H_Bmode);
axis image; colormap gray; colorbar; clim([-dynamicRange 0]);
title(sprintf('B-mode Referencia High a frecuencia %s', freqStr));
xlabel('Posición Lateral (mm)'); ylabel('Profundidad (mm)');

subplot(2,2,3); imagesc(x*1000, z_crop*1000, P_sam_L_Bmode);
axis image; colormap gray; colorbar; clim([-dynamicRange 0]);
title(sprintf('B-mode Muestra Low a frecuencia %s', freqStr));
xlabel('Posición Lateral (mm)'); ylabel('Profundidad (mm)');

subplot(2,2,4); imagesc(x*1000, z_crop*1000, P_sam_H_Bmode);
axis image; colormap gray; colorbar; clim([-dynamicRange 0]);
title(sprintf('B-mode Muestra High a frecuencia %s', freqStr));
xlabel('Posición Lateral (mm)'); ylabel('Profundidad (mm)');

% Guardar como PNG
outNamePNG2 = fullfile(figDir, sprintf('Bmode_%s_mean_angles.png', freqStr));
